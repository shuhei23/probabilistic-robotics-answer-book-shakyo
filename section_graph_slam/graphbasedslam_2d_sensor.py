...
class ObsEdge:
    def __init__(self, t1 , t2, z1, z2, xs, sensor_noise_rate=[0.14, 0.05]): # sensor_noise_rate追加 
        # xs = [step, (x, y, z)]
        # z1 = [id, (ell, phi, psi)]
...
class MapEdge:
    def __init__(self, t, z, head_t, head_z, xs, sensor_noise_rate = [0.14,0.05]): # head_tとhead_zは最初に対象のランドマークを観測したときの時刻とセンサ値
        self.x = xs[t]
        self.z = z
        # ランドマークの位置と向き 式(9.62)
        self.m = self.x[0:2] + np.array([
            z[0] * math.cos(self.x[2] + z[1]),
            z[0] * math.sin(self.x[2] + z[1]) ]).T
...
if __name__ == '__main__':
    ...
    for n in range(1, loop_num): # 繰り返しの回数は適当に大きい値
        ...
        for i in range(len(hat_xs)-1): # 移動エッジの追加
            for j in range(100): # \lamdaの代わり
                edges.append(MotionEdge(i, i+1, hat_xs, us, delta)) # 100回繰り返し = 100倍重くとる

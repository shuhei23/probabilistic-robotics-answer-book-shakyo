...
class Goal:
    ...
    def draw(self, ax, elems):
        x, y = self.pos
        c = ax.scatter(
            x + 0.16, y + 0.5, s=50, marker=">", label="landmarks", color="red"
        )  # 旗
        elems.append(c)
    ...
    def inside(self, pose):
        # distance = math.sqrt(
        #     (self.pos[0] - pose[0]) ** 2 + (self.pos[1] - pose[1]) ** 2
        # )
        # print(
        #     "radius = {0}, self.pos {1}, pose = {2}, distance = {3}".format(
        #         self.radius, self.pos, pose, distance
        #     )
        # )
...
class PuddleIgnoreAgent(EstimationAgent):
    ...
    def policy(cls, pose, goal):
        ...
        direction = int(
            (math.atan2(dy, dx) - theta) * 180 / math.pi
        )  # ゴールの方角(degreeに直す)
        # print("step 1 : {0}".format(self.direction))
        # math.atan2 は　x軸から測った(原点周り)方位角
        # atan2 - theta は　ロボットが自分から見た方位角
        direction = (
            direction + 360 * 1000 + 180
        ) % 360 - 180  # 方角を-180~180[deg]に正規化(適当、ロボットが-1000回転すると破綻)
    ...
    def draw(self, ax, elems):
        ...
        elems.append(
            ax.text(
                x + 1.0, y - 0.5, "reward/sec:" + str(self.reward_per_sec()), fontsize=8
            )
        )
        elems.append(
            ax.text(
                x + 1.0,
                y - 1.0,
                "eval:{:.1f}".format(self.total_reward + self.final_value),
                fontsize=8,
            )
        )

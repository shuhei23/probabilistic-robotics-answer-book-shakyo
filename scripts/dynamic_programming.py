...
class DynamicProgramming:
    def __init__(
        ...
        self.actions = list(
            set([tuple(self.policy[i]) for i in self.indexes])
        ...
        )  # self.index_nums の定義のあとに呼ばないとエラー出る
    ...
    def value_iteration_sweep(self):
        ...
        for index in self.indexes:
            if not self.final_state_flags[index]:
                ...
                max_a = None
                qs = [
                    self.action_value(a, index) for a in self.actions
                ]  # 全行動の行動価値を計算，(状態)行動価値関数
    ...
    def action_value(self, action, index, out_penalty=True):
        value = 0.0
        for delta, prob in self.state_transition_probs[
            (action, index[2])
        ]:  # index[2]:方角のインデックス, action = s' = 移動先のマスのインデックス
            ...
            )  # 最後の　- self.time_interval は移動にかかった時間の計算
    ...
    def init_state_transition_probs(self, time_interval, sampling_num):
        ...
        dx = np.linspace(
            0.001, self.widths[0] * 0.999, sampling_num
        )  # np.linspace(最初の値, 最後の値, 要素数)：np.linspace(0, 10, 3) -> [0.0, 5.0, 10.0]
        ...
        for a in self.actions:  # 各actionに対するfor
            for i_t in range(self.index_nums[2]):  # 各thetaに関するfor
                ...
                for s in samples:
                    # 遷移前の姿勢 (実数)
                    before = (
                        np.array([s[0], s[1], s[2] + i_t * self.widths[2]]).T
                        + self.pose_min
                    )  # self.widths[2]はthetaの幅(10.3.2の例だとmath.pi/18)
                    # 移動前のインデクス
    ...
    def depth_means(self, puddles, sampling_num):
        ...
        for xy in itertools.product(
            range(self.index_nums[0]), range(self.index_nums[1])
        ):  # 空間離散化点に関する for
...
def trial():
    ...
    for i in dp.indexes:
        p[i] = sum(dp.policy[i])  # 速度と各速度を足すと、1.0直進、2.0左回転、-2.0が右回転になる

    sns.heatmap(
        np.rot90(p[:, :, 18]), square=False
    )  # インデックスが18の時 = 180~190degの向きの時の行動を図示
    plt.show()
